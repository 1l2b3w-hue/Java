...
//        先判断是否存在指定文件
//        存在就直接正常运行，如果不存在，则提示只能开始新游戏，并把 key 置为 1
        ...
        switch(key) {
            case "1" :
                ...
                for(int i = 0; i < enemys.size(); i++) {
                    Enemy enemyTank = enemys.get(i);
                    //将 enemys 设置给 enemyTanks
                    enemyTank.setEnemyTanks(enemys);

//            将 enemys 集合 传递给 Recorder
...
//    创建一个方法，显示我方击毁敌方坦克的信息
    public void showInfo(Graphics g) {
        g.setColor(Color.BLACK);
        Font font = new Font("宋体", Font.BOLD, 20);
        g.setFont(font);
        g.drawString("您累积击毁敌方坦克数量",1020,30);
